...
var COMPRESSION_METHOD_BROTLI = "brotli";
function decompress(dataView, compressionMethod, encoding, returnType = "string", decompressConfig) {
...
function parseBox(dataView, offset) {
  ...
  const type = dataView.getUint32(offset + BOX_TYPE_OFFSET2);
  if (type === TYPE_FTYP) {
...
// node_modules/exifreader/src/image-header-jxl.js
var image_header_jxl_default = {
  isJxlFile,
  findJxlOffsets
};
var JXL_CONTAINER_SIGNATURE = [
  0,
  0,
  0,
  12,
...
var JXL_CODESTREAM_SIGNATURE = [255, 10];
var BOX_TYPE_OFFSET = 4;
var BOX_HEADER_MIN_SIZE = 8;
var TYPE_EXIF2 = 1165519206;
var TYPE_XML = 2020437024;
var TYPE_BROB = 1651666786;
var TYPE_JXLC = 1786276963;
var TYPE_JXLP = 1786276976;
var JXLP_SEQUENCE_NUMBER_SIZE = 4;
var BROB_ORIGINAL_TYPE_SIZE = 4;
...
function isJxlCodestream(dataView) {
  ...
  return dataView.getUint8(0) === JXL_CODESTREAM_SIGNATURE[0] && dataView.getUint8(1) === JXL_CODESTREAM_SIGNATURE[1];
}
function findJxlOffsets(dataView) {
  ...
  while (offset + BOX_HEADER_MIN_SIZE <= dataView.byteLength) {
    ...
    if (constants_default.USE_EXIF && boxType === TYPE_EXIF2) {
      try {
        tiffHeaderOffset = getTiffHeaderOffset(dataView, contentOffset);
      } catch (_error) {
      }
    }
    if (constants_default.USE_XMP && boxType === TYPE_XML) {
      xmpChunks = [{
        dataOffset: contentOffset,
        length: length - (contentOffset - offset)
      }];
    }
    if (boxType === TYPE_JXLC && jxlCodestreamOffset === void 0) {
      jxlCodestreamOffset = contentOffset;
    }
    if (boxType === TYPE_JXLP && jxlCodestreamOffset === void 0 && contentOffset + JXLP_SEQUENCE_NUMBER_SIZE <= dataView.byteLength) {
      const sequenceNumber = dataView.getUint32(contentOffset) & 2147483647;
      if (sequenceNumber === 0) {
    ...
    if (boxType === TYPE_BROB && contentOffset + BROB_ORIGINAL_TYPE_SIZE <= dataView.byteLength) {
      ...
      if (constants_default.USE_EXIF && originalType === TYPE_EXIF2 && tiffHeaderOffset === void 0 && !brobExifChunk) {
        brobExifChunk = {
          dataOffset: compressedDataOffset,
          length: compressedDataLength
        };
      }
      if (constants_default.USE_XMP && originalType === TYPE_XML && !xmpChunks && !brobXmpChunk) {
        brobXmpChunk = {
          dataOffset: compressedDataOffset,
          length: compressedDataLength
        };
  ...
  const hasAppMarkers = tiffHeaderOffset !== void 0 || xmpChunks !== void 0 || brobExifChunk !== void 0 || brobXmpChunk !== void 0 || jxlCodestreamOffset !== void 0;
  return {
    hasAppMarkers,
    tiffHeaderOffset,
    xmpChunks,
...
function parseAppMarkers(dataView, async) {
  ...
  if (constants_default.USE_JXL && image_header_jxl_default.isJxlFile(dataView)) {
    return addFileType(image_header_jxl_default.findJxlOffsets(dataView), "jxl", "JPEG XL");
  }
...
function get0thIfdOffset(dataView, tiffHeaderOffset, byteOrder) {
  ...
  if (offset + types_default.getTypeSize("LONG") > dataView.byteLength) {
    return void 0;
  }
  return tiffHeaderOffset + types_default.getLongAt(dataView, offset, byteOrder);
}
...
// node_modules/exifreader/src/jxl-file-tags.js
var jxl_file_tags_default = {
  read: read4
};
var CODESTREAM_SIGNATURE_SIZE = 2;
var SIZE_U32_BITS = [9, 13, 18, 30];
var RATIOS = [0, 1, 12 / 10, 4 / 3, 3 / 2, 16 / 9, 5 / 4, 2];
function read4(dataView, codestreamOffset) {
  const tags = {};
  try {
...
function read6(dataView, dataOffset, includeUnknown, tagFilter = NOOP_TAG_FILTER) {
  try {
...
function read7(dataView, chunks, domParser) {
  const tags = {};
...
function read8(bytes, includeUnknown, tagFilter = NOOP_TAG_FILTER) {
...
function read9(dataView, iccData, async, decompressConfig) {
...
function readCompressedIcc(dataView, iccData, decompressConfig) {
  ...
  return decompress(compressedDataView, iccData[0].compressionMethod, "utf-8", "dataview", decompressConfig).then(parseTags2).catch(() => ({}));
}
...
var CAMERA_SETTINGS_LENS_TYPE = 22;
var canon_tags_default = {
  read: read10,
  SHOT_INFO_AUTO_ROTATE,
  CAMERA_SETTINGS_LENS_TYPE
};
function read10(dataView, tiffHeaderOffset, offset, byteOrder, includeUnknown, computed = false, tagFilter = void 0) {
  let tags = readIfd(
    dataView,
    IFD_TYPE_CANON,
    tiffHeaderOffset,
...
function read11(dataView, tiffHeaderOffset, offset, includeUnknown, computed = false, tagFilter = void 0) {
...
function read12(dataView, fileDataOffset) {
  return {
...
var EXIF_OFFSET = 6;
function read13(dataView, pngTextChunks, async, includeUnknown, computed = false, tagFilter = NOOP_TAG_FILTER, decompressConfig) {
  const tags = {};
  const tagsPromises = [];
...
function read14(dataView, chunkOffsets) {
  const tags = {};
...
function read15(dataView, chunkOffset) {
  const tags = {};
...
function applyMergeStep({
  ...
  if (step.type === "mergeBrobExifDeferred") {
    ...
    const parsedExifTags = deps.filterTagsForParse("exif", resolvedReadTags, tagFilter);
    ...
    const returnedTags = deps.filterTagsForReturn("exif", parsedExifTags, tagFilter);
  ...
  if (step.type === "mergeBrobXmpDeferred") {
    ...
    const parsedXmpTags = deps.filterTagsForParse("xmp", resolvedReadTags, tagFilter);
    parsedGroups.xmp = parsedXmpTags;
    if (!tagFilter.shouldReturnGroup("xmp")) {
      return tags;
    }
    const returnedTags = deps.filterTagsForReturn("xmp", parsedXmpTags, tagFilter);
    if (expanded) {
      tags.xmp = returnedTags;
      return tags;
    }
...
function loadView(dataView, {
  ...
  includeTags = void 0,
  excludeTags = void 0,
  decompress: decompressConfig = void 0
} = {
  expanded: false,
  async: false,
  computed: false,
  ...
  if (constants_default.USE_JXL && constants_default.USE_EXIF && brobExifChunk && !hasExifData(tiffHeaderOffset) && tagFilter.shouldParseGroup("exif") && async) {
    ...
    deferredPromises.push(
      decompress(compressedExifData, COMPRESSION_METHOD_BROTLI, void 0, "dataview", decompressConfig).then((decompressedDataView) => {
        const brobTiffHeaderOffset = getTiffHeaderOffset(decompressedDataView, 0);
        const { tags: readTags2 } = tags_default.read(
        ...
        deferredResults.brobExif = readTags2;
      }).catch(() => {
  ...
  if (constants_default.USE_JXL && constants_default.USE_XMP && brobXmpChunk && !hasXmpData(xmpChunks) && tagFilter.shouldParseGroup("xmp") && async) {
    ...
    deferredPromises.push(
      decompress(compressedXmpData, COMPRESSION_METHOD_BROTLI, void 0, "dataview", decompressConfig).then((decompressedDataView) => {
        deferredResults.brobXmp = xmp_tags_default.read(
          decompressedDataView,
  ...
  if ((constants_default.USE_JPEG || constants_default.USE_WEBP) && constants_default.USE_ICC && hasIccData(iccChunks) && tagFilter.shouldParseGroup("icc")) {
    const readTags2 = icc_tags_default.read(dataView, iccChunks, async, decompressConfig);
  ...
  if (constants_default.USE_JXL && hasJxlCodestreamData(jxlCodestreamOffset) && tagFilter.shouldParseGroup("file")) {
    const readTags2 = jxl_file_tags_default.read(dataView, jxlCodestreamOffset);
    const parsedJxlFileTags = filterTagsForParse("file", readTags2, tagFilter);
...
function hasJxlCodestreamData(jxlCodestreamOffset) {
  return jxlCodestreamOffset !== void 0;
}
export {
