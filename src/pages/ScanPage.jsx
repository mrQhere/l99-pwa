...
import {
  Camera, Upload, RefreshCw, Zap, Search, Phone, User, Users, History, ChevronRight
} from 'lucide-react';
...
export default function ScanPage() {
  ...
  const [irisSize, setIrisSize] = useState(150);
  ...
  return (
    <div className="fade-in">
        ...
        {!selectedPatient ? (
          <>
            ...
            {searchResults.length > 0 && (
              <div style={{ marginTop: 8, border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-md)', background: 'var(--bg-card)', overflow: 'hidden' }}>
                {searchResults.map(p => (
                  <button
                    key={p.id}
                    onClick={() => selectPatient(p)}
                    style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', padding: '12px 16px', background: 'none', border: 'none', borderBottom: '1px solid var(--border-subtle)', color: 'var(--text-primary)', cursor: 'pointer', textAlign: 'left', fontSize: 14, transition: 'background 0.15s' }}
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div className="text-xs text-dim">{p.contact || 'No phone'} · Age {p.age || '?'} · {p.gender || '?'}</div>
                    </div>
            ...
            {phoneSearch.length >= 3 && searchResults.length === 0 && !searching && (
              <div style={{ marginTop: 12 }}>
                <p className="text-xs text-dim">No patients found.</p>
                <button className="btn btn-primary btn-sm" style={{ marginTop: 8 }} onClick={() => navigate('/patients')}>
                  <Users size={14} /> Register New Patient
                </button>
              </div>
            )}
          </>
        ) : (
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: 16, background: 'var(--bg-card)', borderRadius: 'var(--radius-md)', border: '1px solid rgba(0, 240, 255, 0.15)' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div className="text-sm text-dim">{selectedPatient.contact || 'No phone'} · Age {selectedPatient.age || '?'} · {selectedPatient.gender || '?'}</div>
              {selectedPatient.medical_history && (
                <div className="text-xs text-dim mt-8" style={{ fontStyle: 'italic' }}>{selectedPatient.medical_history}</div>
              )}
            </div>
        ...
        {selectedPatient && patientHistory.length > 0 && (
          <div style={{ marginTop: 16 }}>
            <div className="flex items-center gap-8 mb-8">
              ...
              <span className="text-xs text-dim" style={{ textTransform: 'uppercase', letterSpacing: 1 }}>Previous Scans ({patientHistory.length})</span>
            </div>
            <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
              {patientHistory.map(s => (
                <div key={s.id} style={{ padding: '8px 14px', borderRadius: 10, background: 'var(--bg-primary)', border: '1px solid var(--border-subtle)', minWidth: 160, flexShrink: 0 }}>
                  <div className="text-xs text-dim">{new Date(s.created_at).toLocaleDateString()}</div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: severityColors[s.severity_grade] || 'var(--text-primary)', marginTop: 2 }}>{s.diagnosis}</div>
                  <div className="text-xs font-mono" style={{ color: severityColors[s.severity_grade], marginTop: 2 }}>Grade {s.severity_grade} · {s.triage}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── Image Capture ───────────────────────────────── */}
      {!selectedPatient ? (
        <div className="glass-card mb-24" style={{ textAlign: 'center', padding: '40px 20px' }}>
          <div style={{ fontSize: 40, marginBottom: 16 }}>👤</div>
          <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 8, color: 'var(--text-primary)' }}>Select a Patient First</h3>
          <p className="text-sm text-dim" style={{ maxWidth: 340, margin: '0 auto 20px' }}>
            Search for an existing patient above, or register a new patient before capturing a retinal scan.
          </p>
          <button className="btn btn-primary" onClick={() => navigate('/patients')}>
            <Users size={18} /> Go to Patient Registration
          </button>
        </div>
      ) : (
        <div className="glass-card mb-24">
          {!imagePreview && !cameraActive && (
            <div style={{ textAlign: 'center', padding: '40px 20px' }}>
              ...
              <input ref={galleryInputRef} type="file" accept="image/*" onChange={handleFileUpload} style={{ display: 'none' }} />
              ...
              <div className="camera-container">
                ...
                <div className="camera-overlay">
                  <svg width="100%" height="100%" style={{ position: 'absolute', inset: 0 }}>
                    ...
                    <circle cx="50%" cy="50%" r={irisSize / 2} fill="none" stroke="var(--cyan)" strokeWidth="2" strokeDasharray="10 5" style={{ filter: 'drop-shadow(0 0 5px var(--cyan))' }} />
                ...
                <div style={{ position: 'absolute', right: 20, top: '50%', transform: 'translateY(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, background: 'rgba(0,0,0,0.5)', padding: '15px 8px', borderRadius: 20 }}>
                  ...
                  <input type="range" min="100" max="250" value={irisSize} onChange={(e) => setIrisSize(parseInt(e.target.value))} style={{ writingMode: 'bt-lr', WebkitAppearance: 'slider-vertical', width: 8, height: 150 }} />
                  <span style={{ fontSize: 10, fontWeight: 900, color: '#fff' }}>-</span>
                </div>
              </div>
          ...
          {imagePreview && !processing && (
            <div>
              <div style={{ textAlign: 'center', marginBottom: 16 }}>
                <img src={imagePreview} alt="Retinal scan" style={{ maxWidth: '100%', maxHeight: 400, borderRadius: 12, border: '1px solid var(--border-subtle)' }} />
              </div>
              {qualityData && (
              ...
              <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginBottom: 16 }}>
                <div className="search-box" style={{ flex: 1, maxWidth: 220 }}>
                  <select className="input-field" value={manualPriority} onChange={(e) => setManualPriority(e.target.value)} style={{ cursor: 'pointer', appearance: 'auto' }}>
